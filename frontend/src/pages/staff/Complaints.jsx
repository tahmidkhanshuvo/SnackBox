...
const Icon = {
  ...
  Reply: (p) => <svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" strokeWidth="2" {...p}><path d="M4 12h16M4 12l6-6m-6 6l6 6"/></svg>,
};
...
export default function StaffComplaints() {
  ...
  async function fetchComplaints() {
    ...
    setError("");
    try {
      const params = { per_page: 20, q: searchQuery };
  ...
  const handleReply = async (id) => {
    ...
    if (replyText) {
      try {
        await patch(`/api/complaints/${id}/reply`, { reply: replyText });
        fetchComplaints(); // Refresh list
      } catch (err) {
    ...
    <StaffLayout title="Customer Complaints">
      ...
      <div style={{ display:"flex", gap:6, marginBottom:12, alignItems:"center" }}>
        <input
          ...
          style={{ padding:"6px 12px", borderRadius:999, border:"1px solid #e5e7eb", flexGrow:1, maxWidth:"300px" }}
        />
          ...
          <div key={c.id} style={{ border:"1px solid #e5e7eb", borderRadius:12, background:"#fff", padding:16, display:"grid", gridTemplateColumns:"1fr auto", gap:12 }}>
            <div>
              <div style={{display:"flex", alignItems:"center", gap:12, marginBottom:8}}>
                ...
                <div style={{fontSize:14, color:"#6b7280"}}>Complained about: {c.complaint_text}</div>
              </div>
              <div style={{fontSize:12, color:"#6b7280", marginTop:8}}>
                Logged: {new Date(c.created_at).toLocaleString()}
                {c.assigned_to && ` | Assigned to: ${c.assigned_to.name}`}
                {c.response && <div style={{marginTop:8, color:"#4b5563"}}>Response: {c.response}</div>}
              </div>
            </div>
            <div style={{display:"flex", gap:8, alignItems:"center"}}>
              {c.status !== 'Resolved' && (
                <>
                  ...
                  <IconBtn kind="outline" onClick={() => handleReply(c.id)} title="Reply"><Icon.Reply/></IconBtn>
                </>
              )}
            </div>
          </div>
